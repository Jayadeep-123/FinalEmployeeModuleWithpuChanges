...
public interface SkillTestResultRepository extends JpaRepository<SkillTestResult, Integer> {
        ...
        @Query("SELECT r from  SkillTestResult r JOIN FETCH r.skillTestDetlId d LEFT JOIN FETCH r.skillTestApprovalStatus WHERE r.isActive = 1")
        ...
        @Query("SELECT r FROM SkillTestResult r JOIN r.skillTestDetlId d WHERE d.tempPayrollId = :tempPayrollId AND r.isActive = 1 ORDER BY r.examDate DESC")
