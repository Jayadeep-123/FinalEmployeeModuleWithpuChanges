...
public class CentralOfficeLevelService {
    ...
    public CentralOfficeChecklistDTO updateChecklist(CentralOfficeChecklistDTO checklistDTO) {
        ...
        if (checklistDTO.getNoticePeriod() != null && !checklistDTO.getNoticePeriod().trim().isEmpty()) {
            ...
            if (employee.getPayRollId() == null || employee.getPayRollId().trim().isEmpty()) {
                ...
                System.out.println("[WARN] Skipping generation. Employee already has permanent ID: " + employee.getPayRollId());
    ...
    private String generateAndSetPermanentId(Employee employee) {
        ...
        if (masterBase == null) masterBase = 0L;
        ...
        logger.info("Generated next number: {} (Master Base: {}, Employee Max: {})", nextNumber, masterBase, currentEmployeeMax);
        System.out.println("[SERVICE] Generating Payroll ID. Base: " + masterBase + ", EmpMax: " + currentEmployeeMax + ", Next: " + nextNumber);
        ...
        logger.info("Generated permanent payroll ID: {} for emp_id: {} (Organization table not modified)", permanentId, employee.getEmp_id());
    ...
    private void updateEmpSalaryInfoPayrollId(Integer empId, String tempId, String payrollId) {
        ...
        System.out.println("[DIAGNOSTIC] Records for emp_id " + empId + ": Active=" + activeCount + ", Total=" + totalCount);
        if (tempId != null) {
            System.out.println("[DIAGNOSTIC] Records for temp_id " + tempId + ": Active=" + activeTempCount + ", Total=" + totalTempCount);
        }

        if (activeCount == 0 && activeTempCount == 0) {
            System.out.println("[DIAGNOSTIC] ERROR: No active salary record found by emp_id OR temp_id.");
            logger.warn("No active salary record found for emp_id: {} or temp_id: {}. Skipping payroll_id patch.", empId, tempId);
            return;
        }
