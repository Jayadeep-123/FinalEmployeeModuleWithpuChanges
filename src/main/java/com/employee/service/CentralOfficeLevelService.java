...
public class CentralOfficeLevelService {
    ...
    private String generateAndSetPermanentId(Employee employee) {
        ...
        // 3. Resolve Organization from the relationship
        Organization org_data = employee.getOrg_id();
 
        if (org_data == null) {
            logger.error("CRITICAL: Cannot generate permanent ID for emp_id: {}. Organization (org_id) is missing.", employee.getEmp_id());
            throw new ResourceNotFoundException("Cannot generate permanent ID: The Organization (org_id) is missing for employee: " + employee.getEmp_id());
        }
       
        int org_id = org_data.getOrganizationId();
